...
class VideoPrep_MSC_CJ(object):
    def __init__(
        ...
        if augment:
            transforms = [
                video_transforms.RandomResizedCrop(crop, scale=(min_area, 1.0)),
    ...
    def __call__(self, sig, sr, duration=None):
        ...
        if self.trim_pad:
            if sig.shape[0] > num_frames:
                ...
                sig = np.pad(
                    sig, (0, n_pad), mode="constant", constant_values=(0.0, 0.0)
                )

        # Augment by changing volume +/- 10%
        if self.augment:
            sig *= random.uniform(1.0 - self.volume, 1.0 + self.volume)
...
class LogSpectrogram(object):
    ...
    def __call__(self, sig, sr, duration=None):
        ...
        spect = (
            np.abs(librosa.stft(sig[0], n_fft=self.n_fft * 2, hop_length=hop_length))
            ** 2.0
        )
        spect = np.concatenate(
            [spect[:1], spect[1:].reshape(self.n_fft // 2, 2, -1).mean(1)], 0
        )
        ...
        if self.normalize:
            spect = (spect - self.mean[:, np.newaxis]) / (
                self.std[:, np.newaxis] + 1e-5
            )
