...
class VideoDataset(data.Dataset):
    ...
    def _load_sample(self, sample_idx):
        ...
        if self.return_video:
            video_fn = "{}/{}".format(
                self.video_root, self.video_fns[sample_idx].decode()
            )
        ...
        if self.return_audio:
            audio_fn = "{}/{}".format(
                self.audio_root, self.audio_fns[sample_idx].decode()
            )
    ...
    def __getitem__(self, index):
        if self.mode == "clip":
            try:
                ...
                sample = self._get_clip(
                    sample_idx,
                    video_ctr,
                    audio_ctr,
                    v_ss,
                    a_ss,
                    video_clip_duration=v_dur,
                    audio_clip_duration=a_dur,
                )
                if sample is None:
            ...
            if final_time <= start_time:
                final_time = start_time + max(
                    self.video_clip_duration, self.audio_clip_duration
                )
            ...
            sample = self._get_clip(
                ...
                start_time,
                start_time,
                video_clip_duration=video_dur,
                audio_clip_duration=video_dur,
            )
            ...
            if self.return_video:
                ...
                if chunk_size >= nf:
                    chunks["frames"] = torch.stack(
                        [sample["frames"] for _ in range(self.clips_per_video)]
                    )
                else:
                    timestamps = np.linspace(
                        0, max(nf - chunk_size, 1), self.clips_per_video
                    ).astype(int)
                    chunks["frames"] = torch.stack(
                        [sample["frames"][:, ss : ss + chunk_size] for ss in timestamps]
                    )
            ...
            if self.return_audio:
                ...
                if chunk_size >= nf:
                    chunks["audio"] = torch.stack(
                        [sample["audio"] for _ in range(self.clips_per_video)]
                    )
                else:
                    timestamps = np.linspace(
                        0, max(nf - chunk_size, 1), self.clips_per_video
                    ...
                    chunks["audio"] = torch.stack(
                        [sample["audio"][:, ss : ss + chunk_size] for ss in timestamps]
                    )
            ...
            if self.return_index:
                ts = torch.from_numpy(
                    np.linspace(
                        start_time,
                        final_time - self.video_clip_duration,
                        self.clips_per_video,
                    )
                )
                chunks["index"] = torch.stack(
                    [sample["index"][:1].repeat(self.clips_per_video), ts.float()],
                    dim=1,
                )
    ...
    def _sample_snippet(self, video_ctr, audio_ctr):
        video_st, video_ft, audio_st, audio_ft = self._get_time_lims(
            video_ctr, audio_ctr
        )
        if not self.return_audio:
            ...
            if self.video_clip_duration > video_duration:
                ...
                min_d, max_d = self.video_clip_duration, min(
                    self.video_clip_duration, video_duration
                )
            ...
            max_ss = min(
                audio_ft - self.audio_clip_duration, video_ft - self.video_clip_duration
            )
            ...
            if self.audio_clip_duration > self.video_clip_duration:
                ...
                win_max = min(
                    sample_tt_a + self.max_offsync_augm - self.video_clip_duration,
                    video_ft,
                )
                sample_ss_v = random.uniform(win_min, win_max)
                return (
                    sample_ss_v,
                    self.video_clip_duration,
                    sample_ss_a,
                    self.audio_clip_duration,
                )
            else:
                ...
                win_max = min(
                    sample_tt_v + self.max_offsync_augm - self.audio_clip_duration,
                    audio_ft,
                )
