...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    wandb.init(project="avid-ft-ssl", entity="contact-mic", config=cfg)
...
def run_phase(
    phase, loader, model, optimizer, criterion, epoch, args, cfg, logger, tb_writter
):
    ...
    if tb_writter is not None:
        for meter in progress.meters:
            tb_writter.add_scalar(
                "{}-epoch/{}".format(phase, meter.name), meter.avg, epoch
            )
